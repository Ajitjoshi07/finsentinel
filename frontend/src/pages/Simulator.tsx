...
const SCENARIOS = [
  { id: 'random', label: 'Mixed Reality', desc: '85% normal, 15% suspicious — mirrors real-world distribution', color: '#3b82f6', tag: 'Balanced' },
  { id: 'high_fraud', label: 'Fraud Storm', desc: 'High-velocity cross-border attacks and large anomalous amounts', color: '#ef4444', tag: 'Stress test' },
  { id: 'normal', label: 'Clean Traffic', desc: 'Low-risk domestic transactions of a trusted cardholder', color: '#10b981', tag: 'Baseline' },
  { id: 'burst', label: 'Velocity Burst', desc: 'Card testing attack — many transactions in rapid succession', color: '#f59e0b', tag: 'Anomaly' },
];
...
export default function Simulator() {
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 24 }}>Generate synthetic transactions to populate the dashboard and test the fraud engine</p>

      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Choose Scenario</div>
          ...
          {SCENARIOS.map(s => (
            <button key={s.id} onClick={() => setScenario(s.id)} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 14, color: scenario === s.id ? s.color : 'var(--text-primary)' }}>{s.label}</span>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[1, 5, 10, 20].map(n => (
                <button key={n} onClick={() => setCount(n)} style={{
                  padding: '6px 14px', borderRadius: 8, border: '1px solid ' + (count === n ? 'transparent' : 'var(--border)'),
                  background: count === n ? 'var(--accent-primary)' : 'var(--bg-elevated)',
                  color: count === n ? '#fff' : 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'monospace',
                }}>{n}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
            {!autoMode ? (
              ...
              <button onClick={stopAuto} style={{
                padding: '8px 18px', borderRadius: 8, border: '1px solid #ef444466',
                background: '#ef444418', color: '#ef4444', cursor: 'pointer', fontSize: 13, fontWeight: 700,
              }}>⏹ Stop Auto</button>
            )}
            <button onClick={() => run()} disabled={running} style={{
              padding: '8px 24px', borderRadius: 8, border: 'none',
              background: running ? '#666' : sc.color, color: '#fff',
              cursor: running ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700, minWidth: 180,
            }}>
        ...
        {autoMode && (
          <div style={{ marginTop: 12, padding: '10px 14px', background: '#ef444418', border: '1px solid #ef444433', borderRadius: 8, fontSize: 12, color: '#ef4444', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {results.length > 0 && (
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
          ...
          {results.slice(0, 15).map((txn, i) => (
            <div key={i} style={{
              display: 'grid', gridTemplateColumns: '1fr 80px 100px 90px',
              ...
              <div>
                <div style={{ fontWeight: 600, fontSize: 13 }}>{txn.merchant_name || 'Unknown'}</div>
              ...
              <span style={{ alignSelf: 'center', fontSize: 10, padding: '3px 8px', borderRadius: 99, textAlign: 'center', fontFamily: 'monospace',
