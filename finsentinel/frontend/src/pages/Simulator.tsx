...
import { Zap, AlertTriangle, Activity, TrendingUp, Play } from 'lucide-react';
...
const SCENARIOS = [
  {
    id: 'random',
    label: 'Mixed Reality',
    desc: '85% normal, 15% suspicious — mirrors real-world distribution',
  ...
  },
  {
    id: 'high_fraud',
    label: 'Fraud Storm',
    desc: 'High-velocity cross-border micro-transactions and large anomalous amounts',
  ...
  },
  {
    id: 'normal',
    label: 'Clean Traffic',
    desc: 'Low-risk domestic transactions typical of a trusted cardholder',
  ...
  },
  {
    ...
    desc: 'Card testing pattern — many transactions in rapid succession',
    icon: <Zap size={22} />,
    color: 'var(--medium)',
    tag: 'Anomaly',
  },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          Generate synthetic transactions to populate the dashboard and test the fraud engine
        </div>
      </div>
          ...
          {SCENARIOS.map(s => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: s.color, background: s.color + '15', padding: '2px 8px', borderRadius: 99 }}>
                  {s.tag}
                </span>
              </div>
              <div style={{ fontWeight: 700, fontSize: 14, color: scenario === s.id ? s.color : 'var(--text-primary)', marginBottom: 4, fontFamily: 'var(--font-display)' }}>
                {s.label}
              </div>
      ...
      <div className="card">
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
            <button
              ...
              style={{
                borderColor: autoMode ? 'var(--accent-emerald)' : undefined,
                color: autoMode ? 'var(--accent-emerald)' : undefined,
              }}
            >
        ...
        {autoMode && (
          <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, fontSize: 11, color: 'var(--accent-emerald)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span className="live-dot" /> Auto-simulation active · generating 3 transactions every 2 seconds
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)' }}>
              ...
              <span style={{ marginLeft: 10, fontSize: 11, color: 'var(--text-muted)' }}>Most recent {Math.min(results.length, 20)}</span>
            </div>
